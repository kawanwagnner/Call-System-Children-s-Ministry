-- =====================================================
-- Reverter filtro de grupo na recepção
-- Migration: 20251229160000
-- Motivo: Na recepção, os eventos/cultos são abertos para TODOS os membros,
-- independente da célula. O group_id em reception_events é apenas organizacional.
-- =====================================================

-- Corrigir verificar_membros_nao_marcados_recepcao para NÃO filtrar por grupo
...
DECLARE
  ...
  resultado JSON;
BEGIN
  -- Percorrer TODOS os membros (eventos são abertos para todos)
  FOR membro IN 
  ...
  LOOP
    ...
    ELSE
      membros_nao_marcados := array_append(membros_nao_marcados, format('%s (%s)', membro.full_name, COALESCE(membro.group_name, 'Sem célula')));
    END IF;
  END LOOP;
...
-- Garantir que salvar_checkin_completo_recepcao também não filtra por grupo
...
BEGIN
    -- Percorrer TODOS os membros do sistema (eventos são abertos)
    FOR membro IN 
        SELECT m.id, m.full_name, g.name as group_name
        FROM reception_members m
        LEFT JOIN reception_groups g ON g.id = m.group_id
    ...
    LOOP
        ...
        IF p_presencas_marcadas ? membro.id::TEXT THEN
            ...
            ELSE
                ...
                membros_ausentes := array_append(membros_ausentes, format('%s (%s)', membro.full_name, COALESCE(membro.group_name, 'Sem célula')));
            END IF;
        ELSE
...
COMMENT ON FUNCTION verificar_membros_nao_marcados_recepcao IS 'Verifica membros não marcados para eventos de recepção. Processa TODOS os membros pois eventos são abertos.';
COMMENT ON FUNCTION salvar_checkin_completo_recepcao IS 'Salva check-in completo para eventos de recepção. Processa TODOS os membros pois eventos são abertos.';
COMMENT ON COLUMN reception_events.group_id IS 'Grupo organizador do evento (apenas para organização, não restringe participação)';
