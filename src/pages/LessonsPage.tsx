...
import { useLessons, useDeleteLesson } from "../hooks/useLessonQueries";
...
export function LessonsPage({ context = "ministerio" }: LessonsPageProps) {
  ...
  const deleteLesson = useDeleteLesson(context);
                    ...
                    <>
                      {lesson.event_type && (
                        <span
                          className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${
                            lesson.event_type === "Culto"
                              ? "text-blue-600 bg-blue-50"
                              : lesson.event_type === "Reunião"
                      ...
                      {lesson.start_time && (
                        <p className="text-sm text-gray-600">
                          Horário: {lesson.start_time}
                        </p>
                      )}
                      {lesson.leader && (
                        <p className="text-sm text-gray-600">
                          Preletor: {lesson.leader}
                        </p>
                      )}
                    </>
                  ) : (
